...
# Train the UMI policy from the official checkpoint while fine-tuning the
# MAR KL-VAE encoder under shallow DINOv2 feature supervision.
...
CONFIG_NAME="${CONFIG_NAME:-uva_umi_dinov2_trainable_vae_shallow_policy.yaml}"
...
RUN_NAME="${RUN_NAME:-uva_umi_dinov2_shallow_$(date +%Y%m%d_%H%M%S)}"
RUN_DIR="${RUN_DIR:-checkpoints/${RUN_NAME}}"
WANDB_PROJECT="${WANDB_PROJECT:-uva}"
LOG_TAGS="${LOG_TAGS:-[umi,cup_arrangement_0,policy_model,dinov2_vits14_block4,shallow_feature_alignment,trainable_vae_encoder,frozen_vae_decoder,official_umi_pretrained,full_mar_finetune,8gpu]}"
...
echo "DINOv2: frozen ViT-S/14 block 4 teacher"
echo "VAE: trainable encoder/quant-conv; frozen decoder"
echo "MAR: full fine-tuning"
echo "Task mode: policy_model"
echo "Proprioception input/prediction: enabled/enabled"
echo "GPUs: ${GPU_IDS}"
...
launch_args=(
    ...
    model.policy.align_params.coeff="${ALIGN_COEFF}"
